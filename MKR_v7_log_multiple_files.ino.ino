...
const char* file="mkrdat21.txt"; 
const char* file_detail="detail21.txt";
...
void read_and_save_trame_TSEN(){
  ...
  if(r!=TSEN_STOP){
    ...
    save2SD(file,"\n",1);
    // (b)validity + raw + \n
    if(trame_valide){save2SD(file_detail,"1",1);}
      else{save2SD(file_detail,"0",1);}
...
void loop() {
  ////interroger TSEN avec *01A?\r toutes les secondes                           ///// TO_DO : preparation pour la version suivante
  interroge_TSEN(1000);
